...
def body_collision_bounds(species: str, round_pose: bool) -> dict[str, int]:
    ...
    left = x - 16 if species == "robot" else x
    right = x + width + 16 if species == "robot" else x + width
    if species == "bear":
        left = x - 6
        right = x + width + 6
    elif species == "alien":
    ...
    return {
        "x": left,
        "y": top_by_species[species],
        "width": right - left,
        "height": 390 - top_by_species[species],
    }
...
def pack_document(pack_id: str, species: str) -> dict[str, object]:
    return {
        ...
        "layers": [
            ...
            {
                "id": "body-front",
                "source": "layers/10-body-front.svg",
                "z": 10,
                "collision_bounds": body_collision_bounds(species, False),
            },
            {
                "id": "body-round",
                "source": "layers/11-body-round.svg",
                "z": 11,
                "collision_bounds": body_collision_bounds(species, True),
            },
